...
//    }
  ...
  renderTitle = () => {
    ...
    return title
      ? (Platform.OS == 'ios' ? (<View style={styles.title}>
        {this.props.children[this.state.index].props.title}
      </View>) : (<Text numberOfLines={1} style={{width: width - this.state.total * 5, position: 'absolute', left: 10, bottom: 3, fontWeight: 'bold', fontSize: 30 / 2, color: 'white'}}>{this.props.children[this.state.index].props.title.props.children}</Text>))
      : null
  }
...
//    }
    /*
    return (
      <ViewPagerAndroid ref={(ref)=>{this.ScrollView = ref}}
        {...this.props}
      ...
      pages = pages.map((page, i) => {
        if (props.loadMinimal) {
          if (i >= (index + loopVal - props.loadMinimalSize) &&
            ...
            if(loop && i==0) {
                return <View style={pageStyle} key={i}>{children[i]}</View>
            } else {
                return <View style={pageStyle} key={i}>{children[page]}</View>
            }
        }
