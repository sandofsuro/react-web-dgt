  ...
  _createClass(Fastfs, [{
    ...
    value: function build() {
      ...
      return this._crawling.then(function (files) {
        ...
        if (activity) {
          fastfsActivity = activity.startEvent('Building in-memory fs for ' + _this._name);
        }
        files.forEach(function (filePath) {
          var root = _this._getRoot(filePath);
          if (root) {
            var newFile = new File(filePath, false);
            var dirname = filePath.substr(0, filePath.lastIndexOf(path.sep));
            var _parent = _this._fastPaths[dirname];
            _this._fastPaths[filePath] = newFile;
            if (_parent) {
              _parent.addChild(newFile, _this._fastPaths);
            } else {
              root.addChild(newFile, _this._fastPaths);
            }
          }
        });
        if (activity) {
        ...
        if (_this._fileWatcher) {
          _this._fileWatcher.on('all', _this._processFileChange.bind(_this));
        }
      });
    }
  }, {
    ...
    value: function stat(filePath) {
      ...
      return Promise.resolve().then(function () {
        return _this2._getFile(filePath).stat();
      });
    }
  }, {
    key: 'getAllFiles',
    value: function getAllFiles() {
      ...
      return Object.keys(this._fastPaths).filter(function (filePath) {
        return !_this3._fastPaths[filePath].isDir;
      });
    }
  }, {
    key: 'findFilesByExts',
  ...
  _createClass(File, [{
    ...
    value: function readWhile(predicate) {
      ...
      return _readWhile(this.path, predicate).then(function (_ref3) {
        ...
        if (completed && !_this4._read) {
          _this4._read = Promise.resolve(result);
        }
        return result;
      });
    }
